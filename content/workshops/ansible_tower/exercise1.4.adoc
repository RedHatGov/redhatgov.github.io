...
Now use curl to POST the license to the Tower API endpoint. Please edit this command to the use the `workshop_prefix` and `tower_admin_passwd` for your workshop. This command exports your workshop prefix and Tower pasword so that the next command can use that data in the curl command.

[source,bash]
----
export WORKSHOP_PREFIX=" "
export TOWER_ADMIN_PASSWD=" "
----
