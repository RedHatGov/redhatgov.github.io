...
Now use curl to POST the license to the Tower API endpoint. Please edit this command to the use the `WORKSHOP_PREFIX`, and `STUDENT_NUMBER`, for your workshop. This command exports both variables so that the next command can use that data in the curl command.

[source,bash]
----
export WORKSHOP_PREFIX="workshopname "
...
[source,bash]
----
curl -k https://${WORKSHOP_PREFIX}.tower.${STUDENT_NUMBER}.redhatgov.io/api/v1/config/ \
