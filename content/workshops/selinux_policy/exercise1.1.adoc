...
=== Step 3: Changing modes for specific domains
...
Here, we can see that the `httpd` process, ID `2427`, was disallowd from checking attributes of the file `/var/www/html/file1`.  Additionally, we can see that the domain of the process was `httpd_t`, and the file was labeled with the `samba_share_t` context.

=== Step 5: Getting advice
